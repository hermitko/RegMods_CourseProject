...
want to remove a majority effect of "skewness" of the data.
...
varible `mpg` and add them successively to the model.
...
(there is a large gap) and exclude the others, we end with the 
model ``r formula_string``:
...
failed to reject the hypotheses that the type of trasmission
has the impact on car fuel efficiency at 95% confidence level.
...
We present only one figure -- the matrix of one variable
...
```{r message=FALSE, cache=TRUE, fig.width=8, fig.height=7}
library(GGally)
ggpairs(mtcars, 
